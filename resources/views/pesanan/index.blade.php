            ...
            <table class="w-full text-left border-collapse">
                ...
                @forelse($orders as $order)
                    <tr class="border-b border-black hover:bg-gray-50 transition duration-150">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="px-3 py-3 whitespace-nowrap">
                                                                    <form action="{{ route('pesanan.detail.update', $item->id) }}" method="POST">
                                                                        @csrf
                                                                        {{-- UPDATE WARNA DROPDOWN DI POPUP AGAR KONSISTEN --}}
                        ...
                        {{-- STATUS UTAMA (DESKTOP) - SUDAH DIPERBAIKI --}}
                        <td class="p-4 text-center align-middle">
                            @php
                                $statusColor = match($order->status_order) {
                                    'Selesai' => 'bg-[#2ecc71]', // Hijau
                                    'Diambil' => 'bg-blue-500',  // Biru
                                    'Batal'   => 'bg-red-500',   // Merah
                                    default   => 'bg-yellow-500' // Proses/Kuning
                                };
                            @endphp
                            <span class="text-xs font-bold px-4 py-1 rounded-full uppercase tracking-wide shadow-sm text-white {{ $statusColor }}">
                                {{ $order->status_order }}
                            </span>
                        </td>
            ...
            @forelse($orders as $order)
                <div class="bg-white p-4 rounded-xl shadow-md border border-gray-200">
                    <div class="flex justify-between items-start mb-3">
                        ...
                        {{-- STATUS BADGE (MOBILE) - SUDAH DIPERBAIKI --}}
                        @php
                        ...
                        @endphp
                        <span class="{{ $mobileStatusColor }} text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-sm">
                            {{ $order->status_order }}
                        </span>
                    </div>
